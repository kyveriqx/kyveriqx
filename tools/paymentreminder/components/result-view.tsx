...
/* Polls /api/jobs/[id] like the ledgerreco result views and renders the
   send summary: sent / failed counters and a short error table for any
   rows the mailbox rejected. */
...
export function CampaignResultView({ jobId, initialJob }: { jobId: string; initialJob?: Job }) {
  ...
  // with reminder-appropriate wording, so the in-progress experience is
  ...
  if (!job) return <JobProgress stage="queued" title="Getting your reminders ready…" detail="Your reminder run is starting up." />;
  ...
  if (job.status !== "succeeded") {
    ...
    const TITLE: Record<typeof stage, string> = {
      queued: "Getting your reminders ready…",
      running: "Sending your reminders…",
      cancelled: "Reminder run cancelled",
    };
    const DETAIL: Record<typeof stage, string> = {
      queued: "Your reminder run is starting up.",
      running: "Delivering to each customer — this can take a little while.",
    ...
    <div style={{ display: "grid", gap: 24 }}>
      ...
      {res.failed === 0 && res.sent > 0 && (
        <Card style={{ padding: 20, color: "var(--success-fg)", background: "var(--success-bg)", border: "1px solid var(--success-border)" }}>
          ✓ All {res.sent} reminders accepted by your mailbox.
        </Card>
      )}
    </div>
  );
...
function humanizeSendError(raw: string | null): {
  ...
  const e = raw ?? "";
  const SETTINGS = "/tools/paymentreminder?settings=1";
  ...
  if (/535|5\.7\.\d|authentication unsuccessful|invalid login|auth.*fail|basic auth/i.test(e)) {
    return {
      ...
      fix: [
        ...
        "Then send your reminders again.",
      ],
  ...
  if (/reconnect|invalid_grant|refresh token|token (expired|invalid)|unauthor/i.test(e)) {
    return {
      ...
      fix: ["Click “Reconnect mailbox” below and sign in again.", "Then resend your reminders."],
      action: { label: "Reconnect mailbox →", href: SETTINGS },
    };
  }

  // Nothing valid to send to.
  if (/no valid email|customer file is empty|all .* (row|rows).* rejected|empty/i.test(e)) {
    return {
  ...
  return {
    title: "We couldn’t send these reminders",
  ...
  return (
    <Card style={{ padding: 24, borderColor: "var(--warn-border)" }}>
      ...
      <div style={{ display: "flex", alignItems: "center", gap: 14, flexWrap: "wrap" }}>
        ...
        <a href="/tools/paymentreminder" style={{ fontSize: 13.5, color: "var(--ink-400)", textDecoration: "none" }}>
          ← Back to compose
        </a>
      </div>
  ...
  return (
    <Card style={{ padding: 24 }}>
      <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr 1fr", gap: 16 }}>
        <Tile heading="Customers" value={String(res.total)} sub="rows accepted from your file" />
        <Tile heading="Sent" value={String(res.sent)} sub="handed off to your mailbox" tone={res.sent > 0 ? "ok" : "neutral"} />
        <Tile heading="Failed" value={String(res.failed)} sub="rejected by the mail server" tone={res.failed > 0 ? "warn" : "neutral"} />
      </div>
      <div style={{
