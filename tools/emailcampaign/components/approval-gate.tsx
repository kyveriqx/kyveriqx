...
/* Shown on the email campaign tool before a customer is approved to send.
   Email sending is gated by owner approval (anti-abuse / sender reputation):
...
import { requestEmailCampaignAccessAction } from "../request-access-action";
...
export function ApprovalGate({ status, adminNotes }: Props) {
  ...
  function request() {
    ...
    startTransition(async () => {
      const res = await requestEmailCampaignAccessAction();
      ...
      {status === "pending" ? (
        <>
          ...
          <p style={{ color: "var(--ink-300)", fontSize: 14, margin: 0, lineHeight: 1.6 }}>
            Thanks — we&rsquo;ve received your request to use Email Campaigns. To
        ...
        <>
          ...
          <p style={{ color: "var(--ink-300)", fontSize: 14, margin: "0 0 8px", lineHeight: 1.6 }}>
            Email Campaigns access wasn&rsquo;t granted for this account.
        ...
        <>
          <h2 style={{ fontSize: 22, fontWeight: 600, margin: "6px 0 8px", color: "var(--ink-100)" }}>
            Request access to Email Campaigns
          </h2>
          <p style={{ color: "var(--ink-300)", fontSize: 14, margin: "0 0 18px", lineHeight: 1.6 }}>
            ...
            mailbox and send your first campaign.
          </p>
          <Button onClick={request} disabled={pending}>
